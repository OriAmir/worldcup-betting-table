...
// games: per-game 5 חבר'ה breakdown (France-onwards) from fetchSport5Games (optional)
export function mergeStandings(sport5, topScorers = {}, games = []) {
  ...
  const rows = members
    ...
    .map((m) => {
      ...
      const scorerGoalPoints = Number(m.pointsFromScrorer) || 0;

      // Per-game 5 חבר'ה breakdown (France–Senegal onwards) for this player.
      const gameBreakdown = games
        .map((g) => {
          const e = g.perMember[m.name];
          return e
            ? { label: g.label, result: g.result, guess: e.guess, score: e.score }
            : { label: g.label, result: g.result, guess: null, score: 0 };
        });
      const gamesPointsSum =
        Math.round(
          gameBreakdown.reduce((s, g) => s + (g.score || 0), 0) * 100
        ) / 100;
      ...
      return {
        ...
        adjustMissing: adjust && adjust.missing ? adjust.missing : null,
        winnerName,
        winnerFromDominos,
        championRatio,
        topScorerName,
        ...
        liveScorerPoints,
        scorerGoalPoints,
        // breakdown of the 5 חבר'ה column = per-game guesses + scorer points
        gameBreakdown,
        gamesPointsSum,
        winnerCorrect,
        topScorerCorrect,
